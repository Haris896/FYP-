using Proj_Loan_Management.Reports;
using System;
...
{
    public partial class frmDispute : System.Web.UI.Page
    {
        ...
        protected void cmbCustomer_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    //query += " (select l.Loan_id,c.Customer_First_name,l.Loan_Amount,(select Sum(Payment_Amount) from[tbl_payment] where Loan_id = l.Loan_id) PaidAmount,Loan_Application_id from tbl_customer c ";
                    ...
                    //string query = " select Loan_id, Customer_First_name, a.Loan_Application_id,Loan_Amount, Isnull(PaidAmount, 0) PaidAmount,isnull((Loan_Amount - PaidAmount), 0) RemainingAmount1,";
                    //query += " CASE WHEN DATEdIFFERENCE > 0 and Extra_discount!= 0 THEN((DATEdIFFERENCE / 100) * isnull((Loan_Amount - PaidAmount), 0))  ELSE isnull((Loan_Amount - ISNULL(PaidAmount, 0)), 0) END RemainingAmount ";
                    //query += " from( select l.Loan_id, c.Customer_First_name, l.Total_Amount as Loan_Amount, (select Sum(Payment_Amount) from[tbl_payment] where Loan_id = l.Loan_id AND Customer_id = A.Customer_id) PaidAmount,Loan_Application_id, ";
                    //query += " ISNULL((SELECT DATEDIFF(MONTH, (SELECT MAX(payment_date) FROM[tbl_payment] WHERE loan_id = A.Loan_id AND  Customer_id = A.Customer_id), GETDATE())),0) DATEdIFFERENCE, ";
                    //query += " Extra_discount from tbl_customer c ";
                    //query += " inner join tbl_loan_Application A on a.Customer_id = c.Customer_id ";
                    //query += " Inner Join tbl_loan AS l On L.Loan_id = a.Loan_id where a.Customer_id = '" + cmbCustomer.SelectedValue.ToString().Split('-')[0] + "' AND Loan_Application_id = '" + cmbCustomer.SelectedValue.ToString().Split('-')[1] + "'  ) as a ";

                    string query = " select Loan_id, Customer_First_name, a.Loan_Application_id,Loan_Amount, Isnull(PaidAmount, 0) PaidAmount,isnull((Loan_Amount - PaidAmount), 0) RemainingAmount1,";
                    query += " CASE WHEN DATEdIFFERENCE > 0 and Extra_discount!= 0 THEN((DATEdIFFERENCE / 100) * isnull((Loan_Amount - (isnull(PaidAmount,0)+ISNULL(Extra_discount,0))), 0))  ELSE isnull((Loan_Amount - (isnull(PaidAmount,0)+ISNULL(Extra_discount,0))), 0) END RemainingAmount,ISNULL(Extra_discount,0) Extra_discount ";
                    query += " from( select l.Loan_id, c.Customer_First_name, l.Total_Amount as Loan_Amount, (select Sum(Payment_Amount) from[tbl_payment] where Loan_id = l.Loan_id AND Customer_id = A.Customer_id) PaidAmount,Loan_Application_id, ";
                    query += " ISNULL((SELECT DATEDIFF(MONTH, (SELECT MAX(payment_date) FROM[tbl_payment] WHERE loan_id = A.Loan_id AND  Customer_id = A.Customer_id), GETDATE())),0) DATEdIFFERENCE, ";
                    query += " Extra_discount from tbl_customer c ";
                    query += " inner join tbl_loan_Application A on a.Customer_id = c.Customer_id ";
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "INSERT INTO [dbo].[tbl_dispute]([Dispute_TotalAmount],[Dispute_PayableAmount],[payment_date],[loan_id],[Customer_id],[Remarks],Status,Total_PaidAmount) VALUES (@Dispute_TotalAmount,@Dispute_PayableAmount,@payment_date,@loan_id,@Customer_id,@Remarks,@Status,@Total_PaidAmount)";
                    SqlCommand command = new SqlCommand(query, connection);
                    command.Parameters.AddWithValue("@Dispute_TotalAmount", txtloanAmount.Text);
                    command.Parameters.AddWithValue("@Dispute_PayableAmount", txtRemainingAmount.Text);
                    ...
                    command.Parameters.AddWithValue("@Total_PaidAmount", txtPaidAmount.Text);
                    ...
                    LoadNullVal();
                   // blockchain.AddLoanPayment(Convert.ToDecimal(txtRemainingAmount.Text), cmbCustomer.SelectedValue.ToString(), ViewState["Loan_Application_id"].ToString());

                    //BindGridView(cmbCustomer.SelectedValue.ToString(), ViewState["Loan_id"].ToString());
                }
            }
            catch (Exception ex)
            {
